Drop database if exists DBEmpresaMultinacional2018459;
Create database DBEmpresaMultinacional2018459;

Use DBEmpresaMultinacional2018459;


/*
	Nombre: Alan Rene Lopez Lucas
	Curso: Taller
	Código Técnico: IN5AM
    Fecha de Creación: 25-01-2022
    Fechas de Modificación: 27-01-2022
*/
...
Create table Secciones(
    ...
    oficinaId int not null,
    primary key PK_secionId (seccionId),
...
Insert into Regiones (regionID,nombreRegion) values (8,'PetÃ©n');
...
Insert into Oficinas (oficinaID,direccion,departamentoID) values (7001, 'Blv. Los PrÃ³ceres 13-50 zona 10, Ciudad Capital', 1001);
...
Insert into Oficinas (oficinaID,direccion,departamentoID) values (7010, '8va. Calle final Colonia ConcepciÃ³n Zona 3 Retalhuleu.', 1010);
Insert into Oficinas (oficinaID,direccion,departamentoID) values (7011, 'Calzada Miguel GarcÃ­a Granados, Zacapa', 1011);
Insert into Oficinas (oficinaID,direccion,departamentoID) values (7012, 'carretera al pacÃ­fico, Cuyotenango, SuchitepÃ©quez', 1012);
Insert into Oficinas (oficinaID,direccion,departamentoID) values (7013, 'Santa Elena de la Cruz, Flores,El PetÃ©n', 1013);
...
Insert into Secciones (seccionID,nombreSeccion,oficinaID) values (5004,'AtenciÃ³n al Cliente', 7004);
...
Insert into Secciones (seccionID,nombreSeccion,oficinaID) values (5007,'ProducciÃ³n', 7007);
...
Insert into Secciones (seccionID,nombreSeccion,oficinaID) values (5011,'AdministraciÃ³n', 7011);
Insert into Secciones (seccionID,nombreSeccion,oficinaID) values (5012,'Finanzas', 7012);
Insert into Secciones (seccionID,nombreSeccion,oficinaID) values (5013,'AuditorÃ­a', 7013);
Insert into Secciones (seccionID,nombreSeccion,oficinaID) values (5014,'Desarrollo', 7014);
Insert into Secciones (seccionID,nombreSeccion,oficinaID) values (5015,'MecÃ¡nica',7015);
...
Insert into Categorias(categoriaID, nombreCategoria,sueldoBase,bonificacion,bonificacionEmpresa,tipoCategoria) values (4044,'Jefe de secciÃ³n', 8206.29,250.00, 600.00,'Administrativo');
...
Insert into Categorias(categoriaID, nombreCategoria,sueldoBase,bonificacion,bonificacionEmpresa,tipoCategoria) values (4047,'TÃ©cnico', 5978.87,250.00, 300.00,'Administrativo');
...
Insert into Categorias(categoriaID, nombreCategoria,sueldoBase,bonificacion,bonificacionEmpresa,tipoCategoria) values (4052,'MecÃ¡nico', 7549.52,250.00, 1200.00,'Administrativo');
Insert into Categorias(categoriaID, nombreCategoria,sueldoBase,bonificacion,bonificacionEmpresa,tipoCategoria) values (4053,'DiseÃ±ador', 7807.12,250.00, 700.00,'Administrativo');
Insert into Categorias(categoriaID, nombreCategoria,sueldoBase,bonificacion,bonificacionEmpresa,tipoCategoria) values (4054,'PolicÃ­a', 2500.7,250.00, 50.00,'Administrativo');
...
Insert into Empleados (empleadoID,nombre,fechaDeNacimiento,telefonoPersonal,fechaDeContratacion,categoriaID,seccionID) values (138,'Olivia Fernanda QuiÃ±ones Salazar', '1996-09-16', '2534-1777', '2018-01-19',4048,5011);
...
select 2022-year(fechaDeNacimiento) as Edad, count(*) from Empleados group by 2022-year(fechaDeNacimiento) ASC;
...
select * from Empleados where empleadoId = 112;

select
	E.empleadoId,
	E.nombre,
    E.fechaDeNacimiento,
    2022-year(fechaDeNacimiento) as Edad,
    E.telefonoPersonal,
    E.fechaDeContratacion,
    E.categoriaId,
    E.seccionId
    from Empleados E where empleadoId = 112;
...
select empleadoId, nombre, fechaDeNacimiento, 2022-year(fechaDeNacimiento) as Edad, telefonoPersonal,
	fechaDeContratacion, categoriaId from Departamentos D Inner Join Oficinas O
		on D.DepartamentoId = O.DepartamentoId
			Inner Join Secciones S on O.oficinaId = S.oficinaId
				Inner Join Empleados E on S.seccionId = E.seccionId
					where D.departamentoId = 'Chimaltenango';
                    
-- 10) Empleados cuya contratación se produjo en el año X.
select empleadoId, nombre, fechaDeNacimiento, 2022-year(fechaDeNacimiento) as Edad, telefonoPersonal,
	fechaDeContratacion, categoriaId, seccionId from Empleados where year(fechaDeContratacion) = 2015;
    
select * from categorias;

-- 11) Empleados que no sean jefe de sección de la Categoria X.
select empleadoId, nombre, fechaDeNacimiento, 2022-year(fechaDeNacimiento) as Edad, telefonoPersonal,
	fechaDeContratacion, E.categoriaId, E.seccionId, nombreCategoria, nombreSeccion
		from Categorias C Inner Join Empleados E on C.categoriaId = E.categoriaId
...
select empleadoId, nombre, fechaDeNacimiento, 2022-year(fechaDeNacimiento) as Edad, telefonoPersonal,
	fechaDeContratacion categoriaId, seccionId from Empleados where nombre like '%Jose%';
    
select * from Empleados;

-- 17) Empleados que no pertenecen al departamento X.
select empleadoId, nombre, fechaDeNacimiento, 2022-year(fechaDeNacimiento) as Edad, telefonoPersonal,
	fechaDeContratacion, categoriaId from Empleados E Inner Join Secciones S
		on S.seccionId = E.seccionId Inner Join Oficinas O on O.oficinaId = S.oficinaId
			Inner Join Departamentos D on D.departamentoId = O.departamentoId where D.departamento != 'Jutiapa';
            
-- 20) Nombre del empleado y de la categoria en la que trabaja
select nombre, nombreCategoria from Empleados E Inner Join Categorias C
